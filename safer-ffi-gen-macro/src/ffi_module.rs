use crate::{
    utils::{
        add_lifetime_constraint_to_type, has_only_lifetime_parameters, is_cfg,
        is_placeholder_lifetime, new_ident, parent_path, return_type_has_implicit_lifetime,
        specialization_macro_name, string_to_path, LifetimeInserter, TypeReplacer,
    },
    Error, ErrorReason,
};
...
fn into_free_function(
    ...
    //
    // Also add explicit lifetime when `Self` is generic over lifetimes as the code generated by
    // `safer-ffi` may not compile otherwise -- Rust sometimes seems to fail to infer lifetime
    // inter-dependencies.
    if return_type_has_implicit_lifetime(&signature.output) || generics.lifetimes().next().is_some()
    {
        ...
        if let Some(self_lifetime) = self_lifetime {
            if self_lifetime == make_self_lifetime() {
                ...
                // Add lifetime constraint to `Self` type -- somehow Rust does not always infer it
                signature.generics.make_where_clause().predicates.push(
                    add_lifetime_constraint_to_type(literal_self_type(), self_lifetime.clone())
                        .into(),
                );
            }
